...
בלי Azure: המחשב שבו OneDrive מסונכרן מריץ `npm run sales:watch`. הסקריפט שולח את האקסל ל-`/api/sales-dashboard/ingest` כל 5 שניות כשיש שינוי. נדרש אותו `SALES_TV_KIOSK_TOKEN` ב-xCloud וב-`.env.local` של המחשב.
